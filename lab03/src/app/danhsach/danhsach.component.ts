...
export class DanhsachComponent implements OnInit {
  ...
  p: number = 1; // số thứ tự phân trang đang ở
  r=8; // số items xuất hiện trong 1 phân trang
  soft="name"; // Sắp xếp mặc định theo cột name
  reverse: boolean=false // Sắp xếp từ a -> z, min -> max
  constructor(private ActivatedRoute: ActivatedRoute, private orderPipe: OrderPipe) { }

  ngOnInit() {
  }
  // hàm next pagination
   next(){
     if(this.list.length/this.r > this.p){
      this.p++;
     }
   }
   // hàm prev pagination
   ...
   // hàm first pagination
   first(){
      this.p=1;
   }
   // hàm end pagination
   end(){
      var c=this.list.length/this.r;
      if(Number.isInteger(c)){
         this.p=c;
      }else{
         this.p=Math.floor(c)+1;
      }
   }
   // hàm filter theo cột
   filter(s){
